...
fun nivelPeso(p: Persona): String {
    ...
    return when {
        ...
        imc < 24.9 -> "Normal" //cuando el imc está entre 18.5 y 24.9
...
fun ejercicio05b(personas: List<Persona>): List<Persona> {
    ...
    for (persona in personas) {
        if (persona.tieneAutomovil && persona.estrato == 4 &&
            (persona.nivelEducativo == "Pregrado" || persona.nivelEducativo == "Postgrado") &&
            persona.ingresos < 250_000) {
...
fun ejercicio06(personas: List<Persona>): Int {
    val personasPorEstrato = personas.groupBy { it.estrato }
    var mayorCantidadHombres = 0
    var estratoConMasHombres: Int = 0

    for ((estrato, listaPersonas) in personasPorEstrato) {
        val hombresEnEstrato = listaPersonas.count { it.genero == "M" }
        if (hombresEnEstrato > mayorCantidadHombres) {
            mayorCantidadHombres = hombresEnEstrato
            estratoConMasHombres = estrato
        }
    }
    return estratoConMasHombres
}